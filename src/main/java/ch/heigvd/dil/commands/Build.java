...
public class Build implements Callable<Integer> {
    ...
    Integer recursiveBuild(final Path file) throws IOException {
        if (Files.isDirectory(file)) {
            for (Path f : Files.list(file).filter(v -> !v.toAbsolutePath().equals(build.toAbsolutePath())).collect(Collectors.toList())) {
            ...
            if ("md".equals(extension)) {
                ...
                if(separatorIndex == -1) {
                    ...
                    String metadata = fileContent.substring(0, separatorIndex); // Metadata is currently ignored
